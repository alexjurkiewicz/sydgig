...
def update_artist_data(name):
    original_record = controller.get_artist_by_name(name)
    if original_record:
        info = lastfm_api.get_artist_info(name)
        controller.update_artist_by_id(original_record.id, name=info.name, bio=info.bio.content)
    else:
